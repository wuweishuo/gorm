...
func NewCustomSerializer(prefix string) *CustomSerializer {
	return &CustomSerializer{prefix: prefix}
}

func (c *CustomSerializer) Scan(ctx context.Context, field *schema.Field, dst reflect.Value, dbValue interface{}) (err error) {
	switch value := dbValue.(type) {
	case []byte:
		err = field.Set(ctx, dst, bytes.TrimPrefix(value, []byte(c.prefix)))
	case string:
		err = field.Set(ctx, dst, strings.TrimPrefix(value, c.prefix))
	default:
